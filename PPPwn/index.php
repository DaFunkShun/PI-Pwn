...
if (isset($_POST['save'])){
	...
	for($i = 0; $i < count($plst); ++$i) {
        exec('sudo iptables -t nat -I PREROUTING -p tcp --dport '.str_replace("-", ":", $plst[$i]).' -j DNAT --to 192.168.50.2:'.str_replace(":", "-", $plst[$i]));
        exec('sudo iptables -t nat -I PREROUTING -p udp --dport '.str_replace("-", ":", $plst[$i]).' -j DNAT --to 192.168.50.2:'.str_replace(":", "-", $plst[$i]));
	}
